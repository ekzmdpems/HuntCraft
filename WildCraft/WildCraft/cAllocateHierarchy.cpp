...
{
	auto pBone = new ST_BONE;

	pBone->Name = nullptr;
	if (Name)
	{
		auto nLen = strlen(Name) + 1;
	...
	LPD3DXMESHCONTAINER *ppNewMeshContainer)
{
	...
	// 메쉬 컨테이너 생성
	auto pBoneMesh = new ST_BONE_MESH;
	...
	for (DWORD i = 0; i < NumMaterials; ++i)
	{
		...
		auto pTex = TEXTURE_MANAGER->GetTexture(sFullPath);
	...
	if (pAdjacency)
	{
		auto dwNumFaces = pMeshData->pMesh->GetNumFaces();
		pBoneMesh->pAdjacency = new DWORD[3 * dwNumFaces];
		memcpy_s(pBoneMesh->pAdjacency, 3 * sizeof(DWORD) * dwNumFaces, pAdjacency, 3 * sizeof(DWORD) * dwNumFaces);
	}
	...
	if (pSkinInfo)
	{
		...
		auto dwNumBones = pSkinInfo->GetNumBones();
		...
		auto dwOldFVF = pBoneMesh->pWorkingMesh->GetFVF();
		DWORD dwNewFVF =
